let tasks = [
  {
    _id: "5d2ca9e2e03d40b326596aa3",
    completed: true,
    body: "Создать задачу в списке дел",
...
(function (arrOfTasks) {
  ...
  let LastSelectedThem = localStorage.getItem("app_theme") || "default";

  localStorage.setItem("app_tasks", tasks);
  ...
  function onThemSelectHandler(e) {
    const selectedThem = themSelect.value;
    const isConfirmed = confirm("Вы действительно хотите поменять тему?");
    if (!isConfirmed) {
      themSelect.value = LastSelectedThem;
      return;
    }
    setTheme(selectedThem);
    LastSelectedThem = selectedThem;
